...
import { UserWrapper } from './Profiles.styles';
...
import GroupModal from '../../Components/ProfilesModal';
...
const Profiles = () => {
  ...
  const exportCSV = (fileName) => {
    ...
    const csvData = dataGroups.map((user) => ({
      Name: user.name,
      Priority: user.priority,
      Privacy: user.privacy,
      'Edge Type': user.edge_type,
      'Last Updated Time': Number(user.last_updated_time)
        ? dayjs(Number(user.last_updated_time)).format('DD/MM/YYYY')
        : '',
      'last Crawled Time': Number(user.last_crawled_time)
        ? dayjs(Number(user.last_crawled_time)).format('DD/MM/YYYY')
        : '',
      'Created Time': Number(user.created_time) ? dayjs(Number(user.created_time)).format('DD/MM/YYYY') : '',
      'Last Edited Time': Number(user.last_edited_time) ? dayjs(Number(user.last_edited_time)).format('DD/MM/YYYY') : ''
    }));
    ...
    FileSaver.saveAs(data, `Users_${fileName}` + fileExtension);
  };
    ...
    <UserWrapper>
      <div className="header">
        ...
        <div className="menu-right">
          <Export total={totalData} type={'user'} />
